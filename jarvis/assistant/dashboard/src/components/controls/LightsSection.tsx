/**
 * LightsSection — the lights controls extracted from LightsPanel for use
 * inside the new ControlsPanel. Contains: LightOrb toggle, color strip,
 * brightness slider, scene cards. Same visuals, now embedded in a section.
 */
...
function SceneCard({ name, isActive, on, color, onSelect }: {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      animate={{
        borderColor: isActive && on ? `${color}88` : 'var(--border-subtle)',
        boxShadow: isActive && on ? `0 0 16px ${color}30` : '0 2px 8px rgba(0,0,0,0.2)',
      }}
...
export function LightsSection({ store }: Props) {
